...
tf.compat.v1.flags.DEFINE_integer("hidden_layers", 1, "The number of hidden layers of RNN (default: 3)")
...
tf.compat.v1.flags.DEFINE_integer("matrix_type", 0, "The type of adjacency matrix (0=zero,1=real,default:1)")
...
if config.prediction_mode or config.tune_mode:
    with tf.compat.v1.Session(config=gpu_config) as sess:
        ...
        if config.prediction_mode:
            ...
            procedure_CIS.generate_prediction_for_training(net, train_generator1, total_train_batches, config.display_step, item_dict,
        ...
        output_path= output_dir2+ "/prediction2_CIS.txt"
        if config.prediction_mode:
            test_generator = utils.seq_batch_generator(testing_instances, item_dict, config.batch_size, False)
            #test_generator = utils.seq_batch_generator_differently(testing_instances, item_dict, config.batch_size, False)
